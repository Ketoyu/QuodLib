...
namespace QuodLib.WinForms.Objects
{
	public class CForm : Puppeteer<Form>
	{
		...
		public bool Closable
		{
			...
			#endregion //GetSet
			#region ReadOnly
        private Point MousePosition { get { return System.Windows.Forms.Control.MousePosition; } }
			/// <summary>
		...
		public void RecolorTrim(byte hue, byte brightness)
		{
			if (brightness <= 2)
			{
				...
				clrForm[2] = classGraphics.GColor(0, (byte)(10), 0); //edgeFocused

				clrFonts[0] = classGraphics.GColor(0, (byte)(6), 0); //Normal
			}
			...
			if ((IsIn_Toolbar() && MS.IsDown) || MS.IsDragging) {
				...
				if (Maximized) {
					...
					if (mDiff > cfThresh) {
						...
						return new Point(x, MousePosition.Y + (int)(TOOLBAR_HEIGHT / 2));
					} else {
						//throw new Exception("");
